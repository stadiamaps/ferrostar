...
class LocalizedDistanceFormatter(
  ...
  companion object {
    ...
    const val IMPERIAL_CUTOFF = 289

    // Use miles for longer distances (300m is around 0.2mi)
    const val IMPERIAL_WITH_YARDS_CUTOFF = 300

    // Use kilometers above 1km (1000m).
    const val METRIC_CUTOFF = 1_000
  }

  override fun recommendedUnit(distanceInMeters: Double): MeasureUnit {
    ...
    when (measurementSystem) {
      DistanceMeasurementSystem.IMPERIAL -> {
        return if (distanceInMeters > IMPERIAL_CUTOFF) {
          MeasureUnit.MILE
        } else {
          MeasureUnit.FOOT
        }
      ...
      DistanceMeasurementSystem.IMPERIAL_WITH_YARDS -> {
        return if (distanceInMeters > IMPERIAL_WITH_YARDS_CUTOFF) {
          MeasureUnit.MILE
        } else {
          MeasureUnit.YARD
        }
      ...
      DistanceMeasurementSystem.SI -> {
        return if (distanceInMeters > METRIC_CUTOFF) {
  ...
  private fun precision(distanceInMeters: Double): DecimalPrecision =
      when (recommendedUnit(distanceInMeters)) {
        ...
        // Unsupported unit. Keep meters
        else -> DecimalPrecision.NEAREST_INTEGER
      }

  override fun format(distanceInMeters: Double): String {
    val locale = localeOverride ?: ULocale.getDefault(ULocale.Category.FORMAT)
    val unit: MeasureUnit = recommendedUnit(distanceInMeters)
    val distance: Double = roundedDistanceForUnit(distanceInMeters)
    val precision: DecimalPrecision = precision(distanceInMeters)
...
internal fun Double.roundToNearest(wholeNumber: Int): Double {
  return (this / wholeNumber).roundToInt().toDouble() * wholeNumber
}
