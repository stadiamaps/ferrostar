...
use ferrostar::{
    NavigationController, NavigationControllerConfig, Route,
...
fn simple_route_state_machine_manual_advance() {
    ...
    let intermediate_state = controller.update_user_location(user_location_end_of_first_step, &initial_state);
    let TripState::Navigating {
        snapped_user_location,
        ..
    } = intermediate_state.clone() else {
        panic!("Expected state to be navigating");
    };
